...
SampleStrata  <- Wetlands %>%
 ...
 # mutate(YearSampled=0) %>%
  dplyr::select(WTLND_ID, Sampled, SampleType, YearSampled,
                ...
                BEC,
                #Nation,BEC_BCWF,Land_Disturbance,Land_Cover,fire_history,
...
write_sf(SampleStrata, file.path(spatialOutDir,"SampleStrata_TBP.gpkg"))
st_write(SampleStrata, file.path(spatialOutDir,"SampleStrata_TBP.shp"), append=FALSE)
